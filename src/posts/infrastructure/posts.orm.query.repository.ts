...
export class PostsOrmQueryRepository {
  ...
  async findPosts(
    ...
    const posts = await this.dataSource.query(
      `SELECT posts."id", posts."title", posts."shortDescription",
             posts."content", posts."blogId", posts."createdAt", posts."isBanned",
             posts."userId", blogs."name" as blogName
             FROM "posts" as posts
    ...
    const Promises = posts.map(async (p) => {
      ...
      const lastLikes = await this.dataSource.query(
        `SELECT likes.*, users."login" FROM "like_posts" as likes
